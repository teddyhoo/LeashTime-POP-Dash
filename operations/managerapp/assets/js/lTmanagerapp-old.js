        ...
        async function loginPromiseAjax(loginDate) {
            ...
            sitterData = await LTMGR.getManagerDataAjax();
            console.log('Visit data call');
        ...
        function createMapMarker(visitInfo, markerIcon) {
            ...
            if (latitude != null && longitude != null && latitude < 90 && latitude > -90) {
                ...
                el.addEventListener("click", async function(event) {
                    ...
                    vrList()
                    .then((visitReport)=> { 
                        visitReport.forEach((visitReportDetail) => {
                            let visitReportAndDetails;
                            if (visitInfo.visitID == visitReportDetail.visitID) {
                                visitReportAndDetails = visitReportDetail;
                                console.log('matched visit report details');
                            }
                        })
                        return visitReportAndDetails;
                    })
                    .then((popupData)=> {
                        console.log('await data for popup view ' + popupData);
                        //let pBasic = createPopupVisitReportView(visitReportAndDetails);  
                        //popup.setHTML(pBasic);   
                    });

                    if(isAvailable == true) {
                        console.log('creating popup -- is available');
                        let pBasic = createPopupVisitReportView(visitReportAndDetails);  
                        popup.setHTML(pBasic);        
                    }
                   
                       
                                /*let vrDetails = await LTMGR.getVisitReport(visitInfo.visitID);
                                vrDetails.then((vrDetailsDic)=> { 
                                    let pBasic = createPopupVisitReportView(vrDetailsDic, vrListLinks);  
                                    popup.setHTML(pBasic);        
                                    });
                                if (vrItem.status == 'submitted') {
                                    
                                } else if (vrItem.status == 'published') {
                                    vrDetails.then((vrDetailsDic)=> { 
                                        let pBasic = createPopupVisitReportView(vrDetailsDic, vrListLinks);  
                                        popup.setHTML(pBasic);        
                                    });

                                } else if (vrItem.status == 'noreportdatareceived') {
