var LTMGR = (function() {
	...
	class VisitReportVisitDetails {
		constructor(visitListItemDictionary) {
			let keys = Object.keys(visitListItemDictionary);
			keys.forEach((key)=> {
				//console.log(key  + ' --> ' + visitListItemDictionary[key]);
			})
		...
		addVisitDetail(visitDictionary) {
			let keys = Object.keys(visitDictionary);
			keys.forEach((key)=> {
	...
	async function getManagerSittersAjax() {
		...
		console.log('Sitter List')
		sitterList.forEach((sitter)=> {
			console.log('Sitter ID: ' + sitter.id + ', Sitter name: ' + sitter.sitterName);
		})
		return sitterList;
	}
	/*async function getManagerSittersAjax() {
        let url = 'https://leashtime.com/mmd-sitters.php';
        let sitterJSON = await fetch(url).then((response)=> {return response.json();});
        console.log('sitterJSON'+JSON.stringify(sitterJSON));
        sitterList = sitterJSON.sitters;
        console.log('Sitter List');
        sitterList.forEach((sitter)=> {
	...
	function parseVisitReport(visitReportDict) {
		...
		visitReportKeys.forEach((key) => {
			...
			//console.log(key + ' -> ' + visitReportDict[key]);
	...
	async function getManagerClientsAjax() {
		...
		visitList.forEach((visitItem)=> {
			...
			listClientID += visitItem.clientptr + ',';
		});
		console.log(listClientID);

		const options = {
			method : 'POST',
			body : JSON.stringify({'clientids':listClientID}),
			credentials : 'include',
			headers: {
				'Accept': 'application/json',
				'Content-Type' : 'application/json',
	...
	async function getVisitReportAjax(visitID, visitReportDetailObj) {
		let url = 'https://leashtime.com/visit-report-list-ajax.php';
